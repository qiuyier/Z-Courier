...
const routeFileEnvironment = "ZCOURIER_ROUTE_RELOAD_E2E_ROUTE_FILE"
...
func startGatewayProcess(configuration config, routeFile string) (*gatewayProcess, error) {
	...
	environment := replaceEnvironment(os.Environ(), "ZINX_CONFIG_FILE_PATH", configuration.ZinxConfig)
	command.Env = replaceEnvironment(environment, routeFileEnvironment, routeFile)
