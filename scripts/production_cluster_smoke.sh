...
PROMETHEUS_HOST_PORT="${PRODUCTION_CLUSTER_SMOKE_PROMETHEUS_PORT:-19091}"
PROMETHEUS_URL="${PRODUCTION_CLUSTER_SMOKE_PROMETHEUS_URL:-http://127.0.0.1:$PROMETHEUS_HOST_PORT}"
TIMEOUT_SECONDS="${PRODUCTION_CLUSTER_SMOKE_TIMEOUT_SECONDS:-120}"
DOCKER_BUILD_PLATFORM="${ZCOURIER_RELEASE_DOCKER_BUILD_PLATFORM:-}"
SKIP_BUILD="${PRODUCTION_CLUSTER_SMOKE_SKIP_BUILD:-0}"
...
compose() {
  ZCOURIER_PRODUCTION_GATEWAY_A_PORT="$GATEWAY_A_HOST_PORT" \
    ...
    ZCOURIER_PRODUCTION_PROMETHEUS_PORT="$PROMETHEUS_HOST_PORT" \
